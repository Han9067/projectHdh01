...
--장착 부위--
손1,손2,갑옷,신발,투구,장갑,벨트,목걸이,반지1,반지2
...
--스킬 테이블 Att 값 설명--
Att 항목 데이터값 설명
9_1_1_1_1_AvalB -> [특성(Att)ID]_[해당 특성의 초기값]_[특성 활성 레벨]_[레벨별 특성값 증가치]_[몇 레벨당 수치가 증가될지의 레벨 값]_[퍼센트인지 정수인지 체크]
위 데이터를 기반으로 설명: 특성 ID 9번(VIT 증가)를 해당 스킬의 레벨 1일때 활성화하여 1레벨 마다 1씩 증가시킨다.
**이건 모든 특성 관련인데 0만 있으면 아직 더미 데이터 또는 어떠한 효과가 없는 상태이니 예외처리로 해야함